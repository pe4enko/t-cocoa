...
function buildDetailsBlock(report: CocoaReport, signal: TradingSignal): string {
  ...
  const lines = [
    ...
    `${formatDetailValue("Зарубежный тикер", escapeHtml(snapshot.worldClose.symbol))}`,
    `${formatDetailValue("Источник соответствия ICE", escapeHtml(snapshot.worldClose.resolutionSourceLabel ?? "не указан"))}`,
    ``,
    ...
    `${formatDetailValue("Формула", "локальный контракт в $ - зарубежное закрытие")}`,
    ...
    `${formatDetailValue("Формула", "зарубежное закрытие × ставка × дни до экспирации / база_дней")}`,
    ...
    `${formatDetailValue("Формула", "зарубежное закрытие + расчетный базис")}`,
